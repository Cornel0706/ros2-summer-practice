...
class MRTSPExplorerNode(Node):
    def __init__(self) -> None:
        ...
        self.peer_goal_topic = self.declare_parameter(
            "peer_goal_topic", ""
        ).value
        ...
        self.peer_active_target: Optional[np.ndarray] = None
        self.peer_goal_timestamp: Optional[float] = None
        ...
        if self.peer_goal_topic:
            self.create_subscription(
                PoseStamped,
                self.peer_goal_topic,
                self._peer_goal_callback,
                10,
            )
    ...
    def _peer_goal_callback(self, msg: PoseStamped) -> None:
        self.peer_active_target = np.array(
            [msg.pose.position.x, msg.pose.position.y], dtype=float
        )
        self.peer_goal_timestamp = self.get_clock().now().nanoseconds / 1e9

    def _filter_frontiers_by_peer_goal(
        self,
        frontiers: Sequence[Frontier],
    ) -> List[Frontier]:
        if self.peer_active_target is None or self.peer_goal_timestamp is None:
            return list(frontiers)
        
        # Expire peer goal if it's older than 10 seconds
        now = self.get_clock().now().nanoseconds / 1e9
        if now - self.peer_goal_timestamp > 10.0:
            return list(frontiers)

        filtered_frontiers = [
            frontier
            for frontier in frontiers
            if float(np.linalg.norm(frontier.center_array - self.peer_active_target)) >= 2.0
        ]
        
        # If all frontiers are near the peer goal, don't filter them out to prevent deadlock
        ...
        if skipped_count > 0:
            self._log_throttled(
                "peer_frontiers",
                "info",
                "Skipping %d frontiers closer than 2.0m to peer drone's goal"
                % skipped_count,
                period=2.0,
            )
    ...
    def _consider_preempt_active_goal(self, robot_state: RobotState) -> None:
        ...
        frontiers = self._filter_frontiers_by_peer_goal(frontiers)
