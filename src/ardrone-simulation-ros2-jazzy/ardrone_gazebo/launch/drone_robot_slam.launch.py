...
def generate_launch_description():
    ...
    mrtsp_config_1 = os.path.join(pkg_ardrone_gazebo, 'config', 'mrtsp_drone_mixed.yaml')
    ...
    nav2_drone1 = GroupAction([
        PushRosNamespace('drone1'),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(pkg_nav2_bringup, 'launch', 'navigation_launch.py')),
            launch_arguments={'use_sim_time': use_sim_time, 'params_file': nav2_config_1, 'namespace': 'drone1', 'use_collision_monitor': 'False'}.items()
        )
    ])

    mrtsp_drone1 = TimerAction(
        period=10.0,
        actions=[
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(pkg_mrtsp_exploration, 'launch', 'explore.launch.py')),
                launch_arguments={'use_sim_time': use_sim_time, 'params_file': mrtsp_config_1, 'namespace': 'drone1'}.items()
            )
        ]
    )
    ...
    nav2_config_lino = os.path.join(pkg_ardrone_gazebo, 'config', 'nav2_lino.yaml')
    mrtsp_config_lino = os.path.join(pkg_ardrone_gazebo, 'config', 'mrtsp_lino.yaml')
    ekf_config_lino = os.path.join(pkg_ardrone_gazebo, 'config', 'ekf_lino.yaml')
    bridge_config_lino = os.path.join(pkg_ardrone_gazebo, 'config', 'bridge_lino.yaml')
    ...
    nav2_lino = GroupAction([
        PushRosNamespace('lino'),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(pkg_nav2_bringup, 'launch', 'navigation_launch.py')),
            launch_arguments={'use_sim_time': use_sim_time, 'params_file': nav2_config_lino, 'namespace': 'lino', 'use_collision_monitor': 'False'}.items()
        )
    ])

    mrtsp_lino = TimerAction(
        period=10.0,
        actions=[
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(pkg_mrtsp_exploration, 'launch', 'explore.launch.py')),
                launch_arguments={'use_sim_time': use_sim_time, 'params_file': mrtsp_config_lino, 'namespace': 'lino'}.items()
            )
        ]
    )
    ...
    return LaunchDescription([
        ...
        gazebo,
        tf_relay_drone1, tf_relay_lino,
        spawn_drone1, gz_bridge_drone1, drone1_group, nav2_drone1, rtabmap_drone1, mrtsp_drone1,
        spawn_lino, gz_bridge_lino, lino_group, nav2_lino, rtabmap_lino, mrtsp_lino,
        static_tf_drone1, static_tf_lino, map_merger_node, rviz_node
    ])
